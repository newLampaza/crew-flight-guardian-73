  ...
  const startTest = async (testId: string) => {
    try {
      ...
      try {
        ...
        // Более информативная обработка ошибки
        console.error("Ошибка при проверке перезарядки:", cooldownError);
        // Если API перезарядки недоступен, разрешаем пользователю пройти тест,
        // но покажем предупреждение
        toast({
          title: "Предупреждение",
          description: "Не удалось проверить доступность теста. Продолжаем без проверки.",
          variant: "warning"
        });
      }
